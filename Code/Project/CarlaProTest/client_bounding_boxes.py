...
class BasicSynchronousClient(object):
    ...
    def __init__(self):
        ...
        self.parent_img_path = os.path.join('res/screenshots/',
                                            '{}/'.format(datetime.now().date().strftime('%d_%m_%y')))
    ...
    def control(self, car):
        ...
        if keys[K_f]:
            ...
            print("number of vehicle:",len(vehicles))
            ...
            all_boxes_coordinate_vehicles=ClientSideBoundingBoxes.get_bounding_2d_boxes_coordinates(bounding_boxes_vehicles,1)
            all_boxes_coordinate_walkers = ClientSideBoundingBoxes.get_bounding_2d_boxes_coordinates(bounding_boxes_walkers,0)
            ...
            self.save_img(all_boxes_coordinate_vehicles,all_boxes_coordinate_walkers)
        if keys[K_z]:
            control.throttle = 1
    ...
    def save_img(self,coordinates_vehicles,coordinates_walkers):
        ...
        name1 = name + '_edited' + '.jpeg'
        name = name + '.jpeg'


        this_path = os.path.dirname(os.path.abspath(__file__))
        savepath = os.path.join(this_path, self.parent_img_path, '{}'.format(name))
        savepath1=os.path.join(this_path, self.parent_img_path, '{}'.format(name1))
        print("savepath=", savepath)
        print("image type:", type(img1))
        img1.save(savepath, 'JPEG')
        my_image=cv2.imread(savepath)

        array_image=Draw_Box_On_Image.draw_box_1(my_image,coordinates_vehicles,(0,0,255))

        array_image = Draw_Box_On_Image.draw_box_1(array_image, coordinates_walkers,(0,255,0))

        Draw_Box_On_Image.save_image(savepath1,array_image)
